...
const Cart = ({setCartStatus}) =>{
    ...
    const obtenerProductos = async (cart) => {
        let total = 0;
        const carrito = await Promise.all(cart.map(async(e) => {
    ...
    return(
        <div id={style.CartModal}>
            <div id={style.closeButton} onClick={()=>setCartStatus(false)} className={!loading?style.open:undefined}>
                <span></span>
                <span></span>
            </div>
                        ...
                        <div>
                            <div style={{padding:"15px 28px 0 5px"}}>
                                <div id={style.cardsContainer}>
                                    {items.map(e=>(<CardCart key={e._id} name={e.name} img={e.image} price={e.price} brand={e.brand} id={e._id} quantity={e.quantity}/>))}
                                </div>
                            </div>
                        </div>
                    </div>
